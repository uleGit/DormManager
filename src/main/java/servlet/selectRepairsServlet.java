 ...
 */
@WebServlet("/selectRepairsServlet")
public class selectRepairsServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public selectRepairsServlet() {
        super();
		...
		if(s1!=null&&s1.length()>0&&s1.equals("0")) {
			...
			for(int j=0; j<s2.length();j++) {
				...
				if(!Character.isDigit(num[j])) {
					response.sendRedirect("../selectRepair.jsp?error=budno");
					return;
				}
			}
			if(s2!=null&&s2.length()>0) {
				try {
					...
					if(i==Integer.parseInt(s2)) {
						...
						response.sendRedirect("../repairs.jsp");
					}else {
						response.sendRedirect("../selectRepair.jsp?error=budno");
					}
				...
				try {
					...
					if(i==Integer.parseInt(s3)) {
						...
						for(int j=0; j<s4.length();j++) {
							...
							if(!Character.isDigit(num[j])) {
								response.sendRedirect("../selectRepair.jsp?error=dor");
								return;
							}
						}
						if(s4!=null&&s4.length()>0) {
							...
							response.sendRedirect("../selectRepair.jsp?error=dornull");
						}
					}else {
						response.sendRedirect("../selectRepair.jsp?error=budno");
					}
