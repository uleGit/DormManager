...
<body>
<div class="div1" >
    <form action="servlet/deleteStudentServlet" method="post" class="basic-grey">
        <h1>删除学生信息
        	<span>Please fill in the following information!</span>
        </h1>
        <label>
            <span>删除方式 :</span>
            <select name="deleteType" size="1">
            	<option value="sno">学号</option>
            	<option value="dor">宿舍号</option>
            	<option value="cno">班级</option>
            </select>
        </label>
        <label>
            <span>查询码 :</span>
            <input id="deleteNo" type="text" name="deleteNo" placeholder="请输入你所删除的查询码(必填)" />
        </label>
        <label>
            <span>&nbsp;</span>
...
//取出传回来的参数error并与yes比较
  ...
  if(submiti=='yes'){
   ...
   alert("查询不到，删除失败!");
  } else if(submiti=='nonull'){
   alert("查询码不能为空，请输入查询码!");
  }else if(submiti=='dt'){
   alert("查询码格式错误，请重新输入查询码!");
  }
</script>
