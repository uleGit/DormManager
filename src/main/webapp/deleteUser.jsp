...
<body>
<div class="div1" >
    <form action="servlet/deleteUserServlet" method="post" class="basic-grey">
        <h1>删除用户信息
        	<span>Please fill in the following information!</span>
        </h1>
        <label>
            <span>帐号 :</span>
            <input id="deleteNo" type="text" name="deleteNo" placeholder="请输入你所删除的帐号（必填）" />
        </label>
        <label>
            <span>&nbsp;</span>
...
//取出传回来的参数error并与yes比较
  ...
  if(errori=='no'){
   ...
   alert("查询不到，删除失败!");
  } else if(errori=='yesnull'){
   alert("帐号不能为空，请输入帐号!");
  }else if(errori=='s'){
   alert("帐号格式错误，请重新输入帐号!");
  }
</script>
